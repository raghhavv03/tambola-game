// Tiny inline SVG icon set (Lucide outlines, stroke-based) — no icon library
// dependency for six glyphs. Each renders at 1em so it scales with text, and
// is aria-hidden: the BUTTON carries the accessible label, not the picture.
...
function Icon({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <svg
      ...
      aria-hidden="true"
      className={className ?? 'h-[1.2em] w-[1.2em]'}
    >
      {children}
    </svg>
  )
